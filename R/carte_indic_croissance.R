...
carte_indic_croissance<-function(zoom_reg=F,
  ...
  dt<-dt %>%
    mutate(q=cut(part,breaks=bks,labels=round(bks[1:length(bks)-1],1),include.lowest = TRUE) %>%
             fct_explicit_na("Pas de parc récent") %>%
             fct_relevel("Pas de parc récent"))
  ...
  if (zoom_reg==F) {
    ...
    p<-p +
      ...
      labs(title=glue("Poids du parc récent dans l'ensemble du parc social au 1er janvier {params$annee}"),subtitle="En %",fill="")
  }
  return(p)
}
